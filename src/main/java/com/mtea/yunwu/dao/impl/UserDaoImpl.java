...
public class UserDaoImpl implements UserDao {
	 ...
	 */
	@Override
	public Pager<User> findPage(int page, User user) {
...
//		}
		...
		args.add(isEnable ? 1 : 0);
		
		String whereSql = whereBuilder.toString();
		String limitSql = String.format(" LIMIT %s,%s", (page - 1) * Pager.DEFAULT_PAGESIZE, page * Pager.DEFAULT_PAGESIZE);
		long rowCount = this.jdbcTemplate.queryForLong(countSql + whereSql);
		List<User> userList =null;
		if(args.size()==0){
			this.jdbcTemplate.query(querySql + whereSql + limitSql, mapper);
		}else{
			this.jdbcTemplate.query(querySql + whereSql + limitSql, mapper, args);
		}
