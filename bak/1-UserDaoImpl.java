...
public class UserDaoImpl implements UserDao {
	...
	/* (non-Javadoc)
	 * @see com.mtea.yunwu.dao.UserDao#addUser(com.mtea.yunwu.model.User)
	 */
	...
	/* (non-Javadoc)
	 * @see com.mtea.yunwu.dao.UserDao#deleleUser(int)
	 */
	...
	public long updateUserById(User user) throws DaoException{
		SqlArgsBean b=null;
		try {
			b = sqlBuilder.getUpdateSqlArgsBean(user);
		} catch (Exception e) {
			String message = String.format("根据%s实例获得更新的SqlArgsBean出错", modelInspector.getModelClazz().getSimpleName());
			logger.error(message, e);
			throw new DaoException(message);
		}
		return this.jdbcTemplate.update(b.getSql(),b.getArgs());
//		return this.jdbcTemplate.update("UPDATE tb_user SET addTime = ? , editTime = ? , email = ? , isAdmin = ? , isEnable = ? , password = ? , remark = ? , username = ? WHERE id= ?", user.getAddTime(),user.getEditTime(),user.getEmail(),user.getIsAdmin(),user.getIsEnable(),user.getPassword(),user.getRemark(),user.getUsername(),user.getId());
	}
