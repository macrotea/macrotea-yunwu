...
public class BaseController {
	 ...
	 * 默认Action后缀
	 */
	private static final String DEFAULT_ACTION_SUFFIX = "do";
	 ...
	 * 错误视图路径
	 */
	protected String ERROR_VIEW_PATH = "/error";
	...
	protected String redirect(String action) {
		...
		String mappingPath = isAdmin ? "redirect:/admin/%s/%s.%s" : "redirect:/%s/%s.%s";
		return String.format(mappingPath, clazzName, action, DEFAULT_ACTION_SUFFIX);
	}
	...
	protected String bindAction(String action, Object... params) {
		...
		String mappingPath = isAdmin ? "/admin/%s/%s.%s" : "/%s/%s.%s";
		return String.format(mappingPath, clazzName, action + builder.toString(), DEFAULT_ACTION_SUFFIX);
	}
	 ...
	 * @return
	 */
	protected String bindCreateAction(){
